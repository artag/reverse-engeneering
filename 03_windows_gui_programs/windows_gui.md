...
- были удалены вызовы окна Nag путем замены параметров и вызова окна инструкциями `NOP`
- условные jump заменены на безусловные jump с корректировками адресов перехода

## 27. Взлом `CrackMe7.exe` путем правки `EAX`

Файл, который будет ломаться, здесь - [crackMe7.zip](../challenges/crackMe7.zip)

[Описание](../challenges/description.md)

Задание: trace the eax value and patch it.

`EAX` регистры обычно хранят:

- результаты вызова функции
- операции
- сравнения

Операции jump обычно определяются на основе значений `EAX`.

Самостоятельная работа. Моя версия кряка:

- [CrackMe7-edit.zip](src/CrackMe7-edit.zip)

Что сделано:

В `EAX` и `ECX` записывается по 5. Потом `EAX` = `EAX` - `ECX`. И идет условный jump.
Исправил начальное записываемое значение в `EAX` на `6`.

В видео инструкцию `sub eax, ecx` заменили на `mov al, 1` (`AL` это младший регистр `EAX`, занимает 1 байт).
